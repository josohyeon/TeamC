...
try
{
	...
	int rsint   = mStmt.executeUpdate();
     ...
     sql  = "select u_no,no,title,content,hit,wdate,originF,(select nick from users where u_no = board.u_no) as nick from board where no = " + num; //조회수를 올리고 데이터를 가져온다.
	mStmt = conn.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
...
<script>
	function saveFn() 
	{
		...
		({
			...
			success:function(data)
			{
				...
				{	
					output += "<div id='replydiv_"+json[i]["r_no"]+"'>";
					output += "<input type='hidden' id='hiddenTx_"+json[i]["r_no"]+"' value='"+json[i]["r_no"]+"'>";
					output += "<span style ='display:inline-block; width:150px; text-align:center; color:#0064CD;'><b>"+json[i]["nick"]+"</b></span>";
					output += "<span style ='display:inline-block; width:550px;' id='Rcnt_"+json[i]["r_no"]+"'>"+json[i]["r_content"]+"</span>";
					output += "<span style ='display:inline-block; width:150px;'><small>"+json[i]["r_date"]+"</small></span>";
					output += "<span style ='display:inline-block; width:200px;' id='btnSp_"+json[i]["r_no"]+"'><a href='javascript:updateFn("+json[i]["r_no"]+")'>UPDATE</a>";
					output += "&nbsp;&nbsp;<a href='javascript:deleteFn("+json[i]["r_no"]+")'>DELETE</a></span>";
					output += "</div>";
				}
	...
	{	
		...
		var str  = "";
			str += "<span id='contentTx_"+r_no+"'><input type='text' id='textTx_"+r_no+"' value='"+content+"'><input type='hidden' value='"+r_no+"' name='hiddenTx_"+r_no+"' id='hiddenTx_"+r_no+"'></span>"; //수정시 input에 원래 내용 뜨게
	...
	function saveFn2(r_no)
	{
		...
		({
			...
			success:function(data)
			{
				...
				var str = "<a href='javascript:updateFn("+json[0]["r_no"]+")'>UPDATE</a>&nbsp;&nbsp;<a href='javascript:deleteFn("+json[0]["r_no"]+")'>DELETE</a>";
...
<header>
	...
	<article>
		<div class="container" role="main">
				<span class="mt-3" style="margin-bottom:20px; font-size:30px;"><%= v_title %></span>
					<span class="board_date" style="position:absolute; right:450px"><b>조회수</b> <%= v_hit %></span><br>
					<span class="board_date" style="position:absolute; right:450px; top:350px; font-size:20px"><b><%= v_nick %></b></span>
				<hr/>
			<div class="bg-white rounded shadow-sm">
				<div class="board_title"><h4></h4></div>
				<div class="board_info_box">
					<span class="board_author">&nbsp;&nbsp;&nbsp;</span><span class="board_date" style="position:absolute; right:450px"><%= v_wdate %></span><p/>
				</div>
				<div class="board_content" style="height:30vh; margin-top:80px"><font size="4"><%= v_content %></font></div><P/>
			...
			<form id="rep" name="rep">
				...
				<div class="my-3 p-3 bg-white rounded shadow-sm" style="padding-top: 10px">
					<div class="row">
						<div class="col-sm-10">
							<textarea id="content" name="rep1" class="form-control" rows="3" placeholder="댓글을 입력해 주세요"></textarea>
						</div>
						<div class="col-sm-2">
							<%
								if(vnick != null)
								...
								{
								%>
									<button type="button" class="btn btn-sm btn-primary" id="btnReplySave" style="width: 100%; margin-top: 10px" onclick="javascript:alert('로그인 후 이용해주세요.')">SAVE</button>
								<%
								}
							%>
						</div>
			...
			<div class="my-3 p-3 bg-white rounded shadow-sm" style="padding-top: 10px" style="height:100px">
				<h5 class="border-bottom pb-2 mb-0"><b>댓글 목록</b></h5>
				<form id="reply" name="reply">
					<div id="replyList">
						...
						%>
							<div id="replydiv_<%= Rno %>">
								<input type="hidden" id="hiddenTx_<%= Rno %>" name="hiddenTx_<%= Rno %>" value="<%= Rno %>">
								<span style ='display:inline-block; width:150px; text-align:center; color:#0064CD;'><b><%= RNick %></b></span>
								<span style ='display:inline-block; width:550px;' id='Rcnt_<%= Rno%>'><%= Rcontent %></span>
								<span style ='display:inline-block; width:150px;'><small><%= Rdate %></small></span>
								<span style ='display:inline-block; width:200px;' id='btnSp_<%= Rno%>'>
								<%
									if(Uno.equals(vu_no))
									{
									%>
										<a href='javascript:updateFn(<%= Rno%>)'>UPDATE</a>&nbsp;&nbsp;
										<a href='javascript:deleteFn(<%= Rno%>)'>DELETE</a>
									<%
									}else
									{
										%>
