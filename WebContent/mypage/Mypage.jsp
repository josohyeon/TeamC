...
$(document).ready(function()
{
	...
	function checkit()
	{
		var output  = "";
			...
			output +="<span>현재</span><span id='Cal' style='font-weight:bold; color:#B90000'></span><span>일째금연성공하셨습니다!</span>";
			output +="</div><br>";
			output +="<div class='t1'>";
			output +="<div>금연으로 현재까지</div>"		
			output +="<span id='price' style='font-weight:bold; color:#B90000'></span><span>원 절약하셨습니다</span>";
			output +="</div>";
		$("#dday1").html(output);
		...
		var day_span  = document.getElementById("sel_day");
		var fulldate  = sel_year.value+"-"+sel_month.value+"-"+sel_day.value;
		...
		}else if(day >= 600)
		{
			...
			$("#thisis").html(sel_year.value+"년"+sel_month.value+"월"+sel_day.value+"일입니다.");
			$("#Cal").html(day);
...
function jf_setday(day)
{
	...
	day_select += '<select id="sel_day"name="sel_day" style="width:90px; height:22px;">';
	for(var i = 1; i <= daycel; i++)	//daycel까지돌아서 넣어라 10미만의날은 앞에 0을 붙여라
	{
		if(i < 10)
		{
			dayTemp = "0"+i;
		...
		day_select +="<option value='"+dayTemp+"'";	//daytemp가 1부터 지정한 날까지 돌아야되는데 안도네???
		if(i == day)
		{
			day_select +="selected";
		}
		day_select +=">"+dayTemp+"일 </option>";
	}
	day_select += "</select>";
	$("#day_span").html(day_select);	
}
</script>
<style>
input{outline:none;}
	...
	<nav id="navi">
		<div id="menu">
			<ul id="main-menu">
				...
				<li><a href="Mypage.jsp">금연계산기</a></li>
			</ul>
		</div>
	</nav>
	<section>
				...
				<thead>
					...
					<tr id="sap1"><!-- 수정할 부분 -->
						...
						<td colspan="3" class="table table-hover">
							<div class="clock">
			                	<div class="t5">나의 금연 시작일 : </div>
			                	<span id="year_span" name="year_span" ></span>
			                	<span id="month_span" name="month_span"></span>
								<span name="day_span" id="day_span" class="selectpicker" style="width:90px; height:22px;"></span>
							</div>
						</td>
				...
				<tfoot>
					<tr align="center">
						<td  class="note">
							...
							<input class="btn" type="button" value="취소" onclick="location.reload()"> 
						</td>
					</tr>
				</tfoot>
			</table>
